...
import { GET_ALL, GET_SERIES } from "../config/query";
...
export function DetailSeries() {
  ...
  useEffect(() => {
    if (data) {
      setSeriesInput(data.one_series);
    }
  }, [data]);
          ...
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-48">
            ...
            <div className="flex mb-4">
              <span className="text-gray-600 ml-3">
                Popularity: {seriesInput.popularity}
              </span>
            </div>
            <p className="leading-relaxed text-xl">{seriesInput.overview}</p>
            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-200 mb-5"></div>
            <div className="flex ml-20">
              <h2 className="text-gray-600 text-2xl">Genre: </h2>
              <ul className="flex gap-5 ml-5">
                {seriesInput.tags.map((genre) => {
                  return (
